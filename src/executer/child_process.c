...
static void	handle_child_file_redirections(t_msh *msh, t_command *cmd)
{
    ...
    {
        if (cmd->input_redirect[0]) // Get a target for error message if any
...
static void	validate_command_for_execution(t_msh *msh, t_command *cmd)
{
	...
	{
		msh->exit_status = 127;
		exit_process(msh, cmd, cmd->name, "command not found");
	}
	if (is_directory(cmd->path))
	{
		msh->exit_status = 126;
		exit_process(msh, cmd, cmd->path, "Is a directory");
	}
	if (access(cmd->path, X_OK) == -1)
	{
		msh->exit_status = 126;
		exit_process(msh, cmd, cmd->path, "Permission denied");
	}
}
