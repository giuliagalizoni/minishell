...
/*   Updated: 2025/05/25 10:48:17 by giuliagaliz      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	push_split_tokens(char *combined, char ***new_tokens)
{
	...
	// Split only if the token is unquoted
...
static int	handle_mixed_token(char *token, char ***new_tokens, t_msh *msh)
{
	...
	if (ft_strchr(token, '$')) // Expand inline if there's a variable
	{
		...
		expanded = ft_strdup(token); // Copy token if no expansion needed

	// Check if token is quoted
	if (token[0] == '\'' || token[0] == '"')
		processed = remove_quotes(expanded); // Preserve quotes appropriately
	else
		processed = expanded; // No need to remove quotes for unquoted tokens

	// Handle splitting for unquoted expanded tokens
	if (token[0] != '\'' && token[0] != '"')
		success = push_split_tokens(processed, new_tokens);
	else
		success = safe_arr_push(new_tokens, processed);
